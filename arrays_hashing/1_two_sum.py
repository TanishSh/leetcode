...
class Solution:
    def twoSum(self, nums: List[int], target: int) -> List[int]:
        ...
        for i, n in enumerate(nums): # enumerate: index, value
            indices[n] = i  # create array that store index of each num

        for i, n in enumerate(nums):
            diff = target - n
            # diff is 2nd num, it exists in array and the index of the num is not the same as the 1st (i.e. they are unique)
